...
def nama_function():
    ...
    print ("ini adalah blok kode function")
...
```python
def perkenalan (nama, umur, jenis_kelamin, status_lulus):
    print(f"Haii Saya, {nama}")
    print(f"Umur Saya adalah {umur} tahun")
    print(f"Jenis Kelamin Saya adalah {jenis_kelamin}")
    print(f"Status Lulus Saya adalah {status_lulus}")

perkenalan("Andi", 20, "Laki-laki", True)
```

```python
perkenalan( "Hilda", 31, "Perempuan", False)
```
...
print(f"Hasil penjumlahan 5 + 3 adalah {hasil}")
```
...
print(f"Luas segitiga dengan alas 10 dan tinggi 5 adalah: ")
print(luas_segitiga)
```
